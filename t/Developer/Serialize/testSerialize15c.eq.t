...
ok(grep(/Class::AutoDB::Oid/,@refs_oid1)==$OBJECTS,"before eq: objects in list are Oids");
ok(grep(/Class::AutoDB::Oid/,@refs_oid1)==$OBJECTS,"before eq: objects in list1 are Oids");
ok(grep(/Class::AutoDB::Oid/,@refs_oid2)==$OBJECTS,"before eq: objects in list2 are Oids");

my $errors=0;
for (my $i=0;$i<$OBJECTS;$i++) {
  $errors++ unless $objects1[$i] eq $objects2[$i];
  $errors++ if $i!=0 && $objects1[$i] eq $objects[0];
}
ok(!$errors,"examined $OBJECTS objects: $errors eq errors");
my @refs_real=map {ref $_} @objects;
my @refs_real1=map {ref $_} @objects1;
my @refs_real2=map {ref $_} @objects2;
ok(grep(/testSerialize/,@refs_real)==$OBJECTS,"after eq: objects in list are real objects");
ok(grep(/testSerialize/,@refs_real1)==$OBJECTS,"after eq: objects in list1 are real objects");
ok(grep(/testSerialize/,@refs_real2)==$OBJECTS,"after eq: objects in list2 are real objects");
...
for (my $i=0;$i<$OBJECTS;$i++) {
  ...
  $str_errors++ unless  $obj eq $objects[$i];
