...
sub test {
  ...
  for my $reg (@$registers) {
    my @args=@$reg;
    push(@args,(-class=>'testRegistryTop')); # to test class2colls
  ...
  if ($collections) {		# compute from $collections param
    ...
    for my $reg (@$registers) {
      my @args=@$reg;
      my $registration=new Class::AutoDB::Registration(@args);
      my @names=$registration->collections;
      for my $name (@names) {
  ...
  # Test class2colls method (assumes all registrations are on testRegistryTop
  my $class='testRegistryTop';
  my $class_colls=$registry->class2colls('testRegistryTop') || [];
  my @t_names=map {$_->name} @$class_colls;
  cmp_bag(\@t_names,\@names,"$testname: class2colls: names");
  if ($collections) {
    cmp_bag($class_colls,[values %collections],"$testname: class2colls: objects");
  }
...
sub test_inheritance1 {
  ...
  $names or $names=[];
  my $t_collections=$registry->class2colls($class) || [];
  my @t_names=map {$_->name} @$t_collections;
